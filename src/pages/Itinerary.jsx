...
import { Plus, Calendar, MapPin, ArrowRight, Trash2, Clock, MoreVertical, Pin, X } from 'lucide-react';
...
const Itinerary = () => {
    ...
    // Sort trips: Pinned first, then by date (newest first)
    ...
    const handleCreate = (e) => {
        ...
        createTrip({
            ...tripMeta,
            segments, // Pass the multi-city segments
            startDate,
            endDate: end.toISOString(),
            duration: totalDuration,
            destination: segments[0].location // Primary destination
        });
            ...
            <div className="container-custom">
                ...
                {sortedTrips.length === 0 ? (
                    <motion.div
                        ...
                        className="text-center py-20 bg-white dark:bg-slate-800 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700"
                    >
                        ...
                        <AnimatePresence>
                            {sortedTrips.map((trip) => (
                                <motion.div
                                    ...
                                    className={`group relative bg-white dark:bg-slate-800 rounded-2xl shadow-sm border overflow-hidden hover:shadow-xl transition-all duration-300 ${trip.pinned ? 'border-primary-500 ring-1 ring-primary-500' : 'border-slate-200 dark:border-slate-700'}`}
                                >
                                    <div className="h-48 relative overflow-hidden group-hover:h-52 transition-all duration-300">
                                        ...
                                        <div className="absolute top-4 right-4 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                                            <button
                                                ...
                                                className={`p-2 backdrop-blur-md rounded-lg transition-colors ${trip.pinned ? 'bg-primary-500 text-white' : 'bg-white/20 text-white hover:bg-white/40'}`}
                                            ...
                                            <button
                                                ...
                                                className="p-2 bg-white/20 backdrop-blur-md rounded-lg hover:bg-red-500 hover:text-white text-white transition-colors"
                                                title="Delete Trip"
                                            >
                                    ...
                                    <div className="p-6">
                                        ...
                                        <Link
                                            ...
                                            className="inline-flex items-center text-primary-600 dark:text-primary-400 font-medium hover:gap-2 transition-all group-hover:text-primary-700"
                                        >
                    ...
                    {isCreating && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                            ...
                            <motion.div
                                ...
                                className="relative bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-xl p-8 overflow-hidden max-h-[90vh] overflow-y-auto"
                            >
                                <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-primary-500 to-accent-500" />

                                <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-6">Create New Trip</h2>

                                <form onSubmit={handleCreate} className="space-y-6">
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Trip Title</label>
                                        <input
                                            type="text"
                                            required
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">Destinations & Schedule</label>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div className="flex flex-col justify-end pb-2">
                                            <div className="text-sm text-slate-500">
                                                Total Duration: <span className="font-semibold text-slate-900 dark:text-slate-100">{segments.reduce((acc, curr) => acc + curr.days, 0)} days</span>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="relative">
                                                <input
                                                    ...
                                                    onChange={e => setTripMeta({ ...tripMeta, budget: e.target.value })}
                                                />
                                                <span className="absolute left-3 top-2.5 text-slate-500 text-sm">
                                                    {tripMeta.currency === 'INR' ? '₹' :
                                                        tripMeta.currency === 'EUR' ? '€' :
                                                            tripMeta.currency === 'GBP' ? '£' :
                                                                tripMeta.currency === 'JPY' ? '¥' : '$'}
                                                </span>
                                            </div>
                                        </div>
                                        <div>
                                            ...
                                            <select
                                                className="input"
                                                value={tripMeta.currency}
                                                onChange={e => setTripMeta({ ...tripMeta, currency: e.target.value })}
                                            >
                                    ...
                                    <div className="flex gap-4 pt-4">
                                        <button
                                            type="button"
                                            onClick={() => setIsCreating(false)}
                                            className="flex-1 btn bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600"
                                        >
                                            Cancel
                                        </button>
                                        <button
                        ...
                        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                            ...
                            >
                                <div className="text-center">
                                    ...
                                    <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2">Delete Trip?</h3>
                                    ...
                                    <div className="flex gap-3 justify-center">
                                        <button
                                            onClick={() => setTripToDelete(null)}
                                            className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600 font-medium transition-colors"
                                        >
                                            Cancel
                                        </button>
                                        <button
                                            ...
                                            className="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white font-medium transition-colors shadow-lg shadow-red-500/30"
                                        >
                                            Delete Trip
                                        </button>
                                    </div>
