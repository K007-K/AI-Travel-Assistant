...
const Navbar = () => {
    ...
    const NavContainer = ({ children }) => {
        if (scrolled) {
            const darkPill = 'bg-[#030712]/60 border-white/10';
            const lightPill = 'bg-white/70 border-black/5';
            const shadow = 'shadow-[0_8px_32px_rgba(0,0,0,0.08)]';
            
            return (
                <div className={`flex items-center justify-between w-full h-[4.5rem] px-4 md:px-6 transition-all duration-500 backdrop-blur-2xl rounded-full border backdrop-saturate-150 ${shadow} ${isDarkBg ? darkPill : lightPill}`}>
                    {children}
                </div>
            );
        }
        return (
            <div className={`flex items-center justify-between w-full max-w-7xl px-6 h-24 transition-all duration-500 rounded-[2rem] border border-transparent shadow-none backdrop-blur-0 backdrop-saturate-100 ${isDarkBg ? 'bg-[#030712]/0' : 'bg-white/0'}`}>
                {children}
            </div>
        );
    };
    ...
    const logoTextClass = scrolled ? `${baseTextColor} drop-shadow-sm transition-colors duration-300` : `${baseTextColor} drop-shadow-md transition-colors duration-300`;
    ...
    const activeLinkClass = scrolled ? `${baseTextColor} font-bold hover:text-blue-600 transition-colors duration-300` : `${baseTextColor} font-bold drop-shadow-md transition-colors duration-300`;
    
    const loginClass = scrolled ? `${baseTextColor} font-bold hover:text-blue-600 transition-colors duration-300` : `${baseTextColor} font-bold drop-shadow-md hover:${mutedTextColor} transition-colors duration-300`;
