...
const AICenter = () => {
    return (
        <div className="min-h-screen bg-slate-50 relative pb-20">
            {/* 1. STATUS STRIP */}
            <div className="w-full bg-slate-900 border-b border-slate-800 py-1.5 px-6">
            ...
            <div className="pt-12 px-6 max-w-7xl mx-auto">
                ...
                <div className="space-y-12">
                    {CATEGORIES.map((category, catIndex) => {
                        ...
                        return (
                            <div key={category}>
                                <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6 border-l-2 border-blue-500 pl-3">
                                    {category}
                                </h2>
    ...
    return (
        <Link to={feature.link} className="block h-full">
            <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                className={`group relative bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 flex flex-col h-full overflow-hidden cursor-pointer ${feature.primary ? 'ring-1 ring-blue-500/20' : ''}`}
            >
                <div className="p-6 flex-1 flex flex-col">
                    <div className="flex items-start justify-between mb-4">
                        <div className={`p-3 rounded-xl bg-slate-50 group-hover:bg-slate-100 transition-colors ${feature.primary ? 'bg-blue-50 group-hover:bg-blue-100' : ''}`}>
                            {feature.icon}
                        </div>
                        {feature.primary ? (
                            ...
                            <div className="opacity-0 group-hover:opacity-100 transition-opacity text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-1 rounded">
                    ...
                    <div className="mb-5 px-3 py-2 bg-slate-50 rounded-lg text-xs font-medium text-slate-500 flex items-center gap-2">
                        <Settings className="w-3 h-3 text-slate-400" />
                        {feature.capabilities}
                    </div>

                    <div className="pt-4 border-t border-slate-100 mt-auto">
                        <div className="flex items-center justify-between text-[11px] font-medium text-slate-400 mb-4 group-hover:text-slate-600 transition-colors">
                            <span className="truncate max-w-[65%]">
                                {feature.models}
                            </span>

                            <div className="hidden group-hover:flex items-center text-[10px] text-blue-600 font-bold transition-all">
                                {feature.agentChain}
                            </div>
                        </div>

                        <div
                            className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold transition-all duration-300 ${feature.primary ? 'bg-blue-600 text-white hover:bg-blue-700 shadow-blue-600/20 shadow-md' : 'bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 hover:text-slate-900 hover:border-slate-300'}`}
                        >
                            Open Workspace
