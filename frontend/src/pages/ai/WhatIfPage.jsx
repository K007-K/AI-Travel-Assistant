...
export const WhatIfPage = () => {
    ...
    const handleSend = async () => {
        ...
        try {
            const response = await aiManager.runAgent('WHAT_IF_SIMULATION', input);
            const aiContent = `**Impact:** ${response.impact}\n\n**Cost Change:** ${response.changes.cost_delta}\n**Timeline:** ${response.changes.timeline_impact}`;
            setMessages(prev => [...prev, { role: 'ai', content: aiContent }]);
        } catch (error) {
            setMessages(prev => [...prev, { role: 'ai', content: "Simulation failed." }]);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <AIPageLayout
            ...
            subtitle="Explore how small changes impact your entire trip plan."
            ...
            <div className="max-w-3xl mx-auto flex flex-col h-[70vh] bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {messages.length === 0 && (
                        <div className="text-center text-slate-400 mt-20">
                            <Shuffle className="w-16 h-16 mx-auto mb-4 opacity-20" />
                            <p>Try simulating a change: "What if I add a day in Rome?"</p>
                        </div>
                    )}
                    {messages.map((msg, i) => (
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50 grid grid-cols-[1fr,auto] gap-2">
                    ...
                    <button
                        ...
                        className="px-6 bg-slate-900 text-white font-bold rounded-xl hover:bg-cyan-600 transition-colors"
                    >
                        Simulate
                    </button>
                </div>
